...
public class ObjectToCalendar implements Caster<Object, Calendar> {
    ...
    public static Calendar toCalendar(String in, Locale locale) {
        ...
        try {
            ...
            if (sep != '?') {
                ...
                String time = TString.afterIndex(date, sep);
                date = TString.beforeIndex(in, sep).trim();
                parseTime(c, time);
            }
    ...
    public static void parseTime(Calendar c, String time) {
        ...
        // zone
        char sep2 = '?';
        if (TString.isIndex(time, ' ')) sep2 = ' ';
        else if (TString.isIndex(time, '_')) sep2 = '_';
        else if (TString.isIndex(time, 'Z')) sep2 = 'Z';

        if (sep2 == ' ') {
            zone = TString.afterIndex(time, sep2).trim();
            time = TString.beforeIndex(time, sep2).trim();
        ...
        // milliseconds
        if (TString.isIndex(time, '.')) {
            millies = toint(TString.afterLastIndex(time, '.'), 0);
            time = TString.beforeLastIndex(time, '.');
        }

        time = time.trim();
        if (time.length() > 0) {
            String sep3 = null;
            if (TString.isIndex(time, ':')) sep3 = "\\:";
            else if (TString.isIndex(time, '-')) sep3 = "-";
            else if (TString.isIndex(time, '.')) sep3 = "\\.";
