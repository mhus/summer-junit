...
public class DockerScenario {
    ...
    public DockerContainer get(String name) throws TException {
        for (DockerContainer inst : containers) if (inst.getName().equals(name)) return inst;
        throw new TException("container not found", name);
    }
    ...
    public void start() throws InterruptedException {
        ...
        for (DockerContainer cont : containers) {
            ...
            try {
                ...
                docker.pullImageCmd(cont.getImage()).start().awaitCompletion();
            }
            ...
            for (String warning : resp.getWarnings()) System.out.println("    " + warning);
        }
    ...
    public void createNetwork() {
        ...
        if (resNet.getWarnings() != null)
            for (String warning : resNet.getWarnings()) System.out.println("    " + warning);
        networkId = resNet.getId();
    }

    public void destroyNetwork(boolean forced) {

        if (useExistingNetwork && !forced) return;
    ...
    public void destroyPrefix() {
        ...
        for (DockerContainer cont2 : containers) cont2.setId(this, null);
    ...
    public LogStream exec1(String name, String... cmd)
            throws InterruptedException, TException {
        return exec(name, cmd, null, false, null, null, null);
    }

    public LogStream exec(String name, String cmd) throws InterruptedException, TException {
        return exec(name, cmd.split(" "), null, false, null, null, null);
    }

    public LogStream exec(String name, String cmd, String send)
            throws InterruptedException, TException {
        return exec(name, cmd.split(" "), null, false, null, null, send);
    }

    public LogStream exec(
            String name,
            String[] cmd,
            List<String> env,
            boolean privileges,
        ...
        if (cont.getId() == null)
            throw new TException("Container not started", cont.getName());
    ...
    public LogStream exec(
        ...
        ExecCreateCmd builder =
                docker.execCreateCmd(cont.getId())
                        .withAttachStderr(true)
                        .withAttachStdout(true)
                        .withTty(true);

        builder.withCmd(cmd);
        if (send != null) builder.withAttachStdin(true);
        if (env != null) builder.withEnv(env);
        if (privileges) builder.withPrivileged(true);
        if (user != null) builder.withUser(user);
        if (dir != null) builder.withWorkingDir(dir);
        ...
        if (send != null) {
            ByteArrayInputStream is =
                    new ByteArrayInputStream(send.getBytes(TString.CHARSET_CHARSET_UTF_8));
    ...
    public LogStream attach(String name, String send)
            throws InterruptedException, TException {
        DockerContainer cont = get(name);
        if (cont.getId() == null)
            throw new TException("Container not started", cont.getName());
    ...
    public LogStream attach(LogStream stream, String send)
            throws InterruptedException, TException {

        DockerContainer cont = stream.getContainer();

        AttachContainerCmd builder =
                docker.attachContainerCmd(cont.getId())
                        .withStdErr(true)
                        .withStdOut(true)
    ...
    public LogStream logs(String name, boolean follow, int tail) throws TException {
        DockerContainer cont = get(name);
        if (cont.getId() == null)
            throw new TException("Container not started", cont.getName());
    ...
    public LogStream logs(LogStream stream, boolean follow, int tail) throws TException {
        DockerContainer cont = stream.getContainer();
        LogContainerCmd lcc = docker.logContainerCmd(cont.getId());
        lcc.withStdErr(true);
        lcc.withStdOut(true);
        lcc.withFollowStream(follow);
        if (tail > 0) lcc.withTail(tail);
        lcc.exec(stream);
        return stream;
    }

    public void waitForLogEntry(String name, String waitForString, int tail)
    ...
    public boolean waitForLog(
        ...
        if (waitCont.cont.getId() == null)
            throw new TException("Container not started", waitCont.cont.getName());
    ...
    private void fetchContainers() {
        ...
        for (Container cont : docker.listContainersCmd().withShowAll(true).exec()) {
            ...
            for (DockerContainer cont2 : containers) {
                if (contains(cont.getNames(), "/" + prefix + cont2.getName()))
