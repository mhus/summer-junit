    ...
    protected void buildConfig(ContainerBuilder config) {
        ...
        for (String param : params) {
            if (param.equals("privileged")) {
                ...
                config.builder.withUser(TString.afterIndex(param, ':'));
            } else if (param.startsWith("env:") || param.startsWith("e:")) {
                config.env.add(TString.afterIndex(param, ':'));
            } else if (param.startsWith("vol:")
                    || param.startsWith("v:")
                    || param.startsWith("volume:")) {
                config.volumes.add(TString.afterIndex(param, ':'));
                ...
                config.links.add(config.scenario.getPrefix() + TString.afterIndex(param, ':'));
            } else if (param.startsWith("port:") || param.startsWith("p:")) {
                config.ports.add(TString.afterIndex(param, ':'));
            } else if (param.startsWith("cmd:")) {
                config.cmd.add(TString.afterIndex(param, ':'));
            } else if (param.startsWith("entrypoint:")) {
                config.entrypoint.add(TString.afterIndex(param, ':'));
        ...
        for (Map.Entry<ExposedPort, Binding[]> binding :
            ...
            if (binding.getKey().getPort() == exposed) {
                if (binding.getValue().length > 0)
                    return TCast.toint(binding.getValue()[0].getHostPortSpec(), 0);
            }
        }
        return 0;
    }
